...
test("ASIN URL extraction", () => {
  expect(extractAsinFromUrl("http://localhost/api/getAmznPa/B004N3APGO")).toBe(
    "B004N3APGO"
  )
  expect(
    extractAsinFromUrl("https://example.com/api/getAmznPa/1234567890")
  ).toBe("1234567890")
...
test("URL extraction from request", () => {
  const requestWithUrl = new Request(
    "https://example.com/api/getOgp?url=https://test.com"
  )
...
test("Twitter OG image request validation", () => {
  const imageRequest = new Request(
    "https://example.com/post/test-slug/twitter-og.png"
  )
  const nonImageRequest = new Request("https://example.com/post/test-slug")
  const differentImageRequest = new Request(
    "https://example.com/post/test-slug/facebook-og.png"
  )
