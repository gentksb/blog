...
メジャー更新は他グループと混在させず、パッケージごとに個別ブランチ・個別 PR とする。**ただし `engine-pinned-packages.json` に該当するものは major PR も作らず pending に積む（手順 7）**。
...
9. グループが `major:<pkg>` 以外: `pnpm update <pkg1> <pkg2> ...`（semver range 内のみ）
   グループが `major:<pkg>`: devDependencies なら `pnpm add -D <pkg>@<新メジャー>`、dependencies なら `pnpm add <pkg>@<新メジャー>`
...
15. commit 前に `git diff --unified=0 package.json` で、`major:<pkg>` グループ以外のパッケージの major 数字が変化していないか確認する。major の変化を検出したら `git restore package.json pnpm-lock.yaml` で戻し、当該パッケージを pending に積んで次のグループへ進む。

いずれかが失敗した場合: `git reset --hard master` でブランチをリセットし、失敗理由を `pending-updates.json` に記録して次のグループへ進む。

### コミット・PR 作成

16. `git add pnpm-lock.yaml package.json`（src/ の修正があれば追加）
17. `git commit -m "deps(<group-name>): update <list>"`
18. `git push -u origin deps/<group-name>-<YYYYMMDD>`
19. `gh pr create` で PR 作成。本文テンプレート:
...
20. 次のグループへ進む。`master` に戻ってから次のブランチを切る（`git switch master`）。
