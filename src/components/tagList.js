...
const TagList = ({ targetTag }) => {
  ...
  const tagArray =
    (
      allTagAndCount.group.map((tagdata) => {
        ...
        return (
          <Link to={`/tags/${tag.toLowerCase()}`} style={{ textDecoration: 'none' }} key={tag}>
            <TagChip key={tag} label={`${tag}:${count}`} color={isEnabledTag} size="medium" variant="outlined" icon={<LocalOffer />} clickable style={{ textDecoration: `none` }} />
          </Link>
        )
      })
    )
