...
- **UIライブラリ**: React（SocialShare, StickyToc のみ残存）
- **スタイリング**: Tailwind CSS
- **検索機能**: Pagefind
...
例外的に、Amazon PAAPIデータとリンクカードのOGPデータのみ動的に取得している。Astro v6 の Server Islands（`server:defer`）を使い、`LinkCard.astro` と `Amzn.astro` がサーバーサイドで直接 KV キャッシュと外部 API にアクセスする構成。PAAPIデータは規約上24時間以内のデータを配信する必要があるので、Cloudflare KVに24時間TTLでキャッシュしている。サイト内検索についてもPagefindを用いた静的配信を使い、ユーザーアクセス時にサーバサイド処理をなるべく行わない構成としている。
...
- 通常開発： `pnpm dev` Astro v6 + `@astrojs/cloudflare` adapter により、**開発サーバーが workerd（Miniflare）上で動作**。HMR付きで KV バインディング等もローカルシミュレーションで利用可能。記事執筆・UI開発・Server Islands の動作確認に使用。
- 本番ビルド確認： `pnpm dev:cf` `pnpm build` でビルドした成果物を `wrangler dev --config dist/server/wrangler.json` で serve。本番ビルドそのものの動作確認に利用。
- 本番ビルド： `pnpm build` CI/CD デプロイ前の最終確認やビルドエラーの検出に利用。
